    ...
    return (
        <>
            <div className="md:hidden flex items-center justify-between">
                ...
                <button onClick={continueToCheckout} className="flex text-white items-center gap-1 bg-zomato-400 px-3 py-1 rounded-lg">Continue <IoMdArrowDropright /></button>
            </div>
        </>
    );
};
...
const CartLg = ({ toggle }) => {
    ...
    return (
        <>
            ...
            {reduxState.reduce((acc, curVal) => acc + curVal.totalPrice, 0)} </h4>
                    <button onClick={continueToCheckout} className="flex text-white text-lg font-light h-10  items-center gap-1 bg-zomato-400 px-3 py-1 rounded-lg">Continue <IoMdArrowDropright /> </button>
                </div>
            </div>
        </>
    );
